...
#### 主版本（analysis_output_batch）- 基于10天特征集合
...
#### 1120版本（analysis_output_batch_1120）- 基于7天特征集合

经过100种参数组合的批量实验（depth2-depth11，每种depth测试10个leaf值），使用**7天特征集合**（平均气温、日均降雨量、平均相对湿度、累积降雨时数、累积日照时数），强制使用**7天日均降雨量**作为根节点，**7天平均气温**作为深度1节点。确定了三种推荐方案：

| 方案 | 参数组合 | max_depth | min_samples_leaf | 训练准确率 | 验证准确率(2025) | 覆盖样本数 | 综合评分 | 特点 |
|------|---------|-----------|------------------|-----------|-----------------|-----------|----------|------|
| **方案一** | depth4_leaf1 | 4 | 1 | **74.3%** | **69.2%** (9/13) | **35** | **26.00** | 平衡方案 |
| **方案二** | depth8_leaf1 | 8 | 1 | **100.0%** | 46.2% (6/13) | **35** | **35.00** | 综合评分最高 |
| **方案三** | depth3_leaf1 | 3 | 1 | **60.0%** | **69.2%** (9/13) | **30** | **18.00** | 验证准确率最高 |

**推荐说明**：
- **方案一（depth4_leaf1）**：训练准确率74.3%，验证准确率69.2%，各项指标均衡，适合实际应用
- **方案二（depth8_leaf1）**：训练准确率100%，综合评分最高（35.00），但验证准确率较低（46.2%），可能存在过拟合
- **方案三（depth3_leaf1）**：验证准确率最高（69.2%），规则简洁，适合强调验证表现的场景

### 关键发现

1. **最重要气象因子**：
   - **主版本**：10天累积雨量是影响荔枝霜疫霉发生的最关键因子
   - **1120版本**：7天日均降雨量（根节点）和7天平均气温（深度1节点）是最关键因子

2. **预警规则**：成功提取了0-3级预警的阈值规则，规则准确率均达到90%以上

3. **模型性能**：
   - **主版本**：最优模型在57个有效样本上的整体准确率达到93.0%
   - **1120版本**：最优模型在57个有效样本（训练44，验证13）上，训练准确率74.3%-100%，验证准确率46.2%-69.2%

4. **特征集合对比**：
   - **主版本**：使用25个气象因子特征，包括10天、5天、3天累积特征
   - **1120版本**：使用5个7天特征（平均气温、日均降雨量、平均相对湿度、累积降雨时数、累积日照时数），特征更精简，便于实际应用
...
- **1120版本**：`analysis_output_batch_1120/` - 2020年12月20日分析
- **1119版本**：`analysis_output_batch_1119/` - 2020年11月19日分析
- **1118版本**：`analysis_output_batch_1118/` - 2020年11月18日分析
