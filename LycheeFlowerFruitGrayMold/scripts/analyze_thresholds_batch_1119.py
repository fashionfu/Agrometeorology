...
强制使用'7天平均气温'作为决策树的根节点（第一个判断条件）
子节点使用所有特征（包括7天日均降雨量、7天累计降雨时数等）进行详细划分
"""
import argparse
import os
import sys
...
def run_analysis_1119(data_xlsx: str, out_dir: str, max_depth: int = 3, min_samples_leaf: int = 5):
    """运行完整分析流程（基于新的预警标准）"""
    ...
    # 2. 决策树阈值分析（多分类，只考虑1,2,3级，强制使用7天平均气温作为根节点）
    print("\n=== 2. 决策树阈值分析（强制使用'7天平均气温'作为根节点）===")
    
    # 查找强制使用的特征
    forced_feature_name = find_7day_avg_temperature_feature(feature_cols)
    if forced_feature_name is None:
        raise ValueError("未找到'7天平均气温'特征，请检查数据文件")
    
    print(f"强制使用特征作为根节点: {forced_feature_name}")
    print(f"子节点将使用所有特征进行划分: {', '.join(feature_cols)}")
    
    # 构建自定义决策树（强制根节点）
    custom_tree = build_tree_with_forced_root(
        X_train, y_train, feature_cols, forced_feature_name,
...
def generate_report_1119(results: dict, out_dir: str, max_depth: int = 3, min_samples_leaf: int = 5):
    """生成详细分析报告"""
    report_lines = []
    ...
    report_lines.append(f"**决策树多分类（深度 max_depth={max_depth}, min_samples_leaf={min_samples_leaf}）**：使用预警等级（1-3级）作为目标变量，强制使用'7天平均气温'作为根节点，子节点使用所有特征（包括7天日均降雨量、7天累计降雨时数等）进行详细划分，从7天特征中提取每个预警等级的阈值组合规则。\n\n")
    ...
    # 保存报告
    report_path = os.path.join(out_dir, "1119.md")
...
def run_batch_experiments_1119(
    data_xlsx: str,
    base_out_dir: str = "analysis_output_batch_1119"
):
    """运行批量实验（基于新预警标准）"""
    ...
    print("强制使用'7天平均气温'作为决策树的根节点")
    print("子节点使用所有特征（包括7天日均降雨量、7天累计降雨时数等）进行详细划分")
    print("=" * 80)
    ...
    for i, exp in enumerate(experiments, 1):
        ...
        try:
            ...
            # 生成报告
            generate_report_1119(results, out_dir, max_depth=depth, min_samples_leaf=leaf)
    ...
    report_lines.append("- **根节点**: 强制使用'7天平均气温'作为决策树的根节点（第一个判断条件）\n")
    report_lines.append("- **子节点**: 使用所有特征（包括7天日均降雨量、7天累计降雨时数等）进行详细划分\n\n")
    ...
    for exp in experiments:
        ...
        report_lines.append(f"  - 详细报告: `{out_dir}/1119.md`\n\n")
...
def main():
    ...
    default_out = os.path.join(project_root, "analysis_output_batch_1119")
    ...
    run_batch_experiments_1119(args.data, args.out)
